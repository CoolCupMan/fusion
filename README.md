...
| **Legacy → upcoming compatibility** | `minSdk 21` (Android 5.0), `targetSdk 33`; runs on Android 14 too. Signed with the v1+v2 schemes for the widest install range. Both IPv4 and IPv6 are governed. API-gated features (UID attribution, notification channels, usage stats) degrade gracefully on older releases. |
